...
\gloss FAS:SUBJECT CNGR JUSS go FF SSPOSS hut PL PRL
\tr We shall go via your huts
\ex \label{ex:perl2}
...
The intermediate past disjunct suffix \emh{-hi} encodes that an event took place between yesterday (minimum) and one week (maximum) from the time of speech or some reference time. In (\ref{ex:intpast1}), the primary function of the intermediate past is exemplified. In the following, the speaker tells us about his pet armadillo, which escaped from the speaker's hut. 
\begin{examples}
