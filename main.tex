...
The optative mode, marked by the prefix \emh{hó-}, expresses wishes and hopes, however there is no appeal to the addressee to make the action true. Some examples of the usage of the optative mode are shown in examples (\ref{ex:opt1}) and (\ref{ex:opt2}).
\begin{examples}
    \ex \label{ex:opt1}
    \words pói hóhooxáa\MC3
    \bits pói hó- hooxá -a
    \gloss SS:SUBJECT OPT be.glad ST
    \tr May you live merrily!
    \ex \label{ex:opt2}
    \words pípíò\MC2 hóxàa\MC2
    \bits pípí -ò hó- xàa
    \gloss rich CL:anim.sg OPT be 
